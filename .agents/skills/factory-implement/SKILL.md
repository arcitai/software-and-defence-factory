...
Publishing a branch or PR, merging, deploying and sending messages follow the project's separate authorities. This skill grants none of them. Use an existing explicit mandate or configured delivery policy without asking again; hand off actions outside the worker's authority. The optional Arcitai local starter currently grants no external-write authority to its worker.
