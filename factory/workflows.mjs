...
import { ROOT, digest } from './lib.mjs';
...
const phaseInfo = {
  build: { title: 'Build', owner: 'agent', skills: ['factory-implement'], description: 'Implement the accepted scope in an isolated checkout. Produce a candidate and evidence.' },
...
export function workflowDefinitions(config) {
  ...
  return {
    ...
    commands: Object.entries(phaseInfo).map(([name, info]) => ({ name, ...info, prompt: info.description,
      executor: info.owner === 'agent' ? config.agent : 'factory', timeout: `${config.timeoutSeconds}s` })),
    skills,
    configuration: { agent: config.agent, model: config.model || null, check: config.check, timeoutSeconds: config.timeoutSeconds,
