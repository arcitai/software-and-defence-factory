...
export function RunComposer({ title, setTitle, sourceURL, setSourceURL, choices, identity, selection, setSelection, repository, prompt, setPrompt, model, setModel, submitting, submit, close, csrfToken, projectLinks, error }) {
  ...
  const [mode, setMode] = useState('brief'), [issueURL, setIssueURL] = useState(''), [loading, setLoading] = useState(false), [importError, setImportError] = useState(''), [imported, setImported] = useState(false);
  ...
  return <dialog ref={dialog} className="work-dialog" aria-labelledby="start-work-title" aria-describedby="start-work-description" onCancel={event => { event.preventDefault(); if (!submitting) close(); }}>
    <header className="work-dialog-header"><div><h2 id="start-work-title">New task</h2><p id="start-work-description">{repositoryLabel(repository, identity)}</p></div><Button variant="ghost" size="icon" onClick={close} disabled={submitting} aria-label="Close start work form"><X size={18} /></Button></header>
    <form onSubmit={event => { if (mode === 'issue' && !imported || !prompt.trim() || submitting) { event.preventDefault(); return; } submit(event); }}>
      ...
      <footer className="work-dialog-footer"><p>Starts work with this project's settings.</p><Button disabled={submitting || loading || !prompt.trim() || mode === 'issue' && !imported || !selection || !repository}>{submitting ? 'Starting…' : 'Start task'}<Play size={14} /></Button></footer>
    </form>
  </dialog>;
}
