...
try {
  save(join(state,'factory.json'),{...original,model:'synthetic-profile-a',command:['node','--input-type=module','-e',noisy]});
  ...
  assert.equal(first.execution.requestedModel,'synthetic-profile-a');assert.equal(first.execution.runtimeVersion,VERSION);
  ...
  save(join(state,'factory.json'),{...original,model:'synthetic-profile-b'});
  await cli('retry',id);job=await wait(id,'awaiting_approval');
  const second=job.runs.find(r=>r.command==='build'&&r.id!==first.id);
  assert.deepEqual(job.runs[0],first);assert.equal(second.model,'synthetic-profile-b');assert.notEqual(second.execution.policyHash,first.execution.policyHash);
